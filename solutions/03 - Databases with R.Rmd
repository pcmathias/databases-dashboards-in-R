...
1. Open the R Markdown file for this lesson: “03 – Databases in R.Rmd”
...
3. Run the “import” chunk next. For the other exercises in this lesson we will be using a subset of the peak, sample, and batch data sets to build and query our database.
...
testdb <- dbConnect(RSQLite::SQLite(), "test_database.sqlite", extended_types = TRUE)
```

```{r write_table}
# write tables
dbWriteTable(testdb, "batch", batch_file)
dbWriteTable(testdb, "sample", sample_file)
dbWriteTable(testdb, "peak", peak_file)
# confirm tables
dbListTables(testdb)
```
...
sample_sql <- tbl(testdb, "sample")
head(sample_sql, 10)
peak_sql <- tbl(testdb, "peak")
head(peak_sql, 10)
```
